...
for ip in $BK_APPT_IP_COMMA;do
    ...
    ssh $ip 'source /data/install/utils.fc;exec 2>&1;docker exec vault sh -c "export VAULT_ADDR=http://127.0.0.1:8200 && vault operator unseal ${VAULT_UNSEAL_CODE}" && echo "[green]解锁成功[white]" || echo "[red]解锁失败[white]"'
done