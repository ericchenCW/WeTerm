...
func TestSignature_RabbitMQQueueNameDrift(t *testing.T) {
	// Real-world sample: bk_bkmonitorv3 stalled queue set rotated between
	// the 22:44 and 23:02 inspection runs, but the underlying problem is the
	// same vhost.
	at2244 := []AlertItem{
		{Field: "rabbitmq.bk_bkmonitorv3.celery_api_cron.stalled"},
		{Field: "rabbitmq.bk_bkmonitorv3.celery_cron.stalled"},
		{Field: "rabbitmq.bk_bkmonitorv3.celery_service.stalled"},
	}
	at2302 := []AlertItem{
		{Field: "rabbitmq.bk_bkmonitorv3.celery_alert_builder.stalled"},
		{Field: "rabbitmq.bk_bkmonitorv3.celery_cron.stalled"},
		{Field: "rabbitmq.bk_bkmonitorv3.celery_service_access_event.stalled"},
	}
...
func TestSignature_RabbitMQQueueCountChange(t *testing.T) {
	three := []AlertItem{
		{Field: "rabbitmq.bk_bkmonitorv3.q1.stalled"},
		{Field: "rabbitmq.bk_bkmonitorv3.q2.stalled"},
		{Field: "rabbitmq.bk_bkmonitorv3.q3.stalled"},
	}
	five := []AlertItem{
		{Field: "rabbitmq.bk_bkmonitorv3.q1.stalled"},
		{Field: "rabbitmq.bk_bkmonitorv3.q2.stalled"},
		{Field: "rabbitmq.bk_bkmonitorv3.q3.stalled"},
		{Field: "rabbitmq.bk_bkmonitorv3.q4.stalled"},
		{Field: "rabbitmq.bk_bkmonitorv3.q5.stalled"},
	}
...
func TestSignature_RabbitMQCrossVhost(t *testing.T) {
	a := []AlertItem{
		{Field: "rabbitmq.vhostA.q1.stalled"},
	}
	b := []AlertItem{
		{Field: "rabbitmq.vhostA.q1.stalled"},
		{Field: "rabbitmq.vhostB.q1.stalled"},
	}
	if Signature(a) == Signature(b) {
...
func TestSignature_RabbitMQBacklogVsStalledDistinct(t *testing.T) {
	a := []AlertItem{{Field: "rabbitmq.vhostA.q1.stalled"}}
	b := []AlertItem{{Field: "rabbitmq.vhostA.q1.backlog"}}
	if Signature(a) == Signature(b) {
		t.Fatalf("backlog and stalled must not collapse together")
	}
}

func TestSignature_LegacyNoConsumerFieldNotFolded(t *testing.T) {
	// If a stray legacy `no_consumer` field ever surfaces, the new regex must
	// NOT silently absorb it into the vhost-level stalled key — keeping the
	// raw form makes the drift obvious in logs / dedup state.
	legacy := []AlertItem{{Field: "rabbitmq.vh.q1.no_consumer"}}
	stalled := []AlertItem{{Field: "rabbitmq.vh.q1.stalled"}}
	if Signature(legacy) == Signature(stalled) {
		t.Fatalf("legacy no_consumer field must not collapse onto the new stalled key")
	}
}
