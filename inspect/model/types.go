...
// MessageStatus colors the message-count cell, ConsumerStatus the consumer
// cell. AckRate / PublishRate are the rolling per-second counters surfaced by
// the RabbitMQ management API (message_stats.ack_details.rate /
// publish_details.rate); they drive the "stalled" detection rule. When the
// upstream API omits message_stats (e.g. a queue that has never seen
// deliver/ack activity), both rates are 0.0 and the stalled rule's
// publish_rate>min clause naturally exempts the queue.
...
type RabbitMQStatus struct {
	...
	StalledQueues       []RabbitMQQueue        `json:"stalled_queues"`
