...
type Thresholds struct {
	...
	// "Stalled" queue detection: a queue is stalled when messages>0 AND
	// ack_rate < RabbitMQStalledAckRateMax AND publish_rate > RabbitMQStalledPublishRateMin.
	// Setting RabbitMQStalledPublishRateMin to a negative value disables the
	// publish-rate clause entirely (only ack-rate then matters).
	RabbitMQStalledAckRateMax     float64
	RabbitMQStalledPublishRateMin float64
	// vhosts whose queues are exempt from the stalled alert (queue backlog
	// alert still applies). env-set value fully replaces the default.
	RabbitMQStalledVHostBlacklist []string
...
func Load(outputDir string) (*Config, error) {
	...
	rmqStalledAckMax, err := parsePositiveFloatEnv("INSPECT_RABBITMQ_STALLED_ACK_RATE_MAX", 0.01)
	if err != nil {
		return nil, err
	}
	rmqStalledPubMin, err := parseFloatEnv("INSPECT_RABBITMQ_STALLED_PUBLISH_RATE_MIN", 0.0)
	if err != nil {
		return nil, err
	}
	...
	c.Thresholds = Thresholds{
		...
		RabbitMQStalledAckRateMax:     rmqStalledAckMax,
		RabbitMQStalledPublishRateMin: rmqStalledPubMin,
		RabbitMQStalledVHostBlacklist: parseStalledVHostBlacklist([]string{"bk_bknodeman"}),
...
// parseStalledVHostBlacklist parses the stalled-queue vhost blacklist with
// full-replace semantics. Lookup order:
//  1. INSPECT_RABBITMQ_STALLED_VHOST_BLACKLIST (preferred). Set, even to "",
//     fully replaces the default — "" disables the blacklist entirely.
//  2. INSPECT_RABBITMQ_NO_CONSUMER_VHOST_BLACKLIST (legacy, deprecated). Same
//     semantics. A stderr deprecation warning is emitted when this path is
//     taken so operators know to migrate.
//  3. Default.
func parseStalledVHostBlacklist(def []string) []string {
	if v, present := os.LookupEnv("INSPECT_RABBITMQ_STALLED_VHOST_BLACKLIST"); present {
		return splitVHostList(v)
	}
	if v, present := os.LookupEnv("INSPECT_RABBITMQ_NO_CONSUMER_VHOST_BLACKLIST"); present {
		fmt.Fprintln(os.Stderr,
			"config: INSPECT_RABBITMQ_NO_CONSUMER_VHOST_BLACKLIST 已弃用，"+
				"请改用 INSPECT_RABBITMQ_STALLED_VHOST_BLACKLIST（语义相同，下个版本移除旧变量）")
		return splitVHostList(v)
	}
	return def
}

func splitVHostList(val string) []string {
	var out []string
...
// parsePositiveFloatEnv is parseFloatEnv with the extra constraint that the
// parsed value MUST be > 0. Returns an error when the env value is non-numeric
// or non-positive.
func parsePositiveFloatEnv(key string, def float64) (float64, error) {
	f, err := parseFloatEnv(key, def)
	if err != nil {
		return 0, err
	}
	if f <= 0 {
		return 0, fmt.Errorf("invalid float for %s: %v (must be > 0)", key, f)
	}
	return f, nil
}
