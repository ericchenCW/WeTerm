...
func TestRabbitMQStalled_Defaults(t *testing.T) {
	t.Setenv("BK_PAAS_IP_COMMA", "10.0.0.1")
	for _, k := range []string{
		"INSPECT_RABBITMQ_STALLED_ACK_RATE_MAX",
		"INSPECT_RABBITMQ_STALLED_PUBLISH_RATE_MIN",
		"INSPECT_RABBITMQ_STALLED_VHOST_BLACKLIST",
		"INSPECT_RABBITMQ_NO_CONSUMER_VHOST_BLACKLIST",
	} {
		unsetEnv(t, k)
	}
	c, err := Load("/tmp")
	...
	if c.Thresholds.RabbitMQStalledAckRateMax != 0.01 {
		t.Errorf("ack rate default = %v, want 0.01", c.Thresholds.RabbitMQStalledAckRateMax)
	}
	if c.Thresholds.RabbitMQStalledPublishRateMin != 0.0 {
		t.Errorf("publish rate min default = %v, want 0.0", c.Thresholds.RabbitMQStalledPublishRateMin)
	}
	want := []string{"bk_bknodeman"}
	if !reflect.DeepEqual(c.Thresholds.RabbitMQStalledVHostBlacklist, want) {
		t.Errorf("default blacklist = %v, want %v", c.Thresholds.RabbitMQStalledVHostBlacklist, want)
	}
}

func TestRabbitMQStalledAckRateMax_EnvOverride(t *testing.T) {
	t.Setenv("BK_PAAS_IP_COMMA", "10.0.0.1")
	t.Setenv("INSPECT_RABBITMQ_STALLED_ACK_RATE_MAX", "0.5")
	c, err := Load("/tmp")
	if err != nil {
		t.Fatalf("Load: %v", err)
	}
	if c.Thresholds.RabbitMQStalledAckRateMax != 0.5 {
		t.Errorf("ack rate = %v, want 0.5", c.Thresholds.RabbitMQStalledAckRateMax)
	}
}

func TestRabbitMQStalledAckRateMax_RejectsNonPositive(t *testing.T) {
	t.Setenv("BK_PAAS_IP_COMMA", "10.0.0.1")
	for _, bad := range []string{"0", "-1", "abc"} {
		t.Setenv("INSPECT_RABBITMQ_STALLED_ACK_RATE_MAX", bad)
		if _, err := Load("/tmp"); err == nil {
			t.Errorf("Load should fail for ack rate %q", bad)
		}
	}
}

func TestRabbitMQStalledPublishRateMin_NegativeDisablesClause(t *testing.T) {
	t.Setenv("BK_PAAS_IP_COMMA", "10.0.0.1")
	t.Setenv("INSPECT_RABBITMQ_STALLED_PUBLISH_RATE_MIN", "-1")
	c, err := Load("/tmp")
	if err != nil {
		t.Fatalf("Load: %v", err)
	}
	if c.Thresholds.RabbitMQStalledPublishRateMin != -1 {
		t.Errorf("publish rate min = %v, want -1", c.Thresholds.RabbitMQStalledPublishRateMin)
	}
}

func TestRabbitMQStalledVHostBlacklist_EnvOverride(t *testing.T) {
	t.Setenv("BK_PAAS_IP_COMMA", "10.0.0.1")
	unsetEnv(t, "INSPECT_RABBITMQ_NO_CONSUMER_VHOST_BLACKLIST")
	t.Setenv("INSPECT_RABBITMQ_STALLED_VHOST_BLACKLIST", "foo, bar ,baz")
	c, err := Load("/tmp")
	if err != nil {
		t.Fatalf("Load: %v", err)
	}
	...
	if !reflect.DeepEqual(c.Thresholds.RabbitMQStalledVHostBlacklist, want) {
		t.Errorf("env override = %v, want %v", c.Thresholds.RabbitMQStalledVHostBlacklist, want)
	}
}

func TestRabbitMQStalledVHostBlacklist_EmptyDisablesBlacklist(t *testing.T) {
	t.Setenv("BK_PAAS_IP_COMMA", "10.0.0.1")
	unsetEnv(t, "INSPECT_RABBITMQ_NO_CONSUMER_VHOST_BLACKLIST")
	t.Setenv("INSPECT_RABBITMQ_STALLED_VHOST_BLACKLIST", "")
	c, err := Load("/tmp")
	if err != nil {
		t.Fatalf("Load: %v", err)
	}
	if len(c.Thresholds.RabbitMQStalledVHostBlacklist) != 0 {
		t.Errorf("empty env should disable blacklist, got %v", c.Thresholds.RabbitMQStalledVHostBlacklist)
	}
}

func TestRabbitMQStalledVHostBlacklist_LegacyVarFallback(t *testing.T) {
	t.Setenv("BK_PAAS_IP_COMMA", "10.0.0.1")
	unsetEnv(t, "INSPECT_RABBITMQ_STALLED_VHOST_BLACKLIST")
	t.Setenv("INSPECT_RABBITMQ_NO_CONSUMER_VHOST_BLACKLIST", "legacy_vhost")
	c, err := Load("/tmp")
	if err != nil {
		t.Fatalf("Load: %v", err)
	}
	want := []string{"legacy_vhost"}
	if !reflect.DeepEqual(c.Thresholds.RabbitMQStalledVHostBlacklist, want) {
		t.Errorf("legacy fallback = %v, want %v", c.Thresholds.RabbitMQStalledVHostBlacklist, want)
	}
}

func TestRabbitMQStalledVHostBlacklist_NewVarWinsOverLegacy(t *testing.T) {
	t.Setenv("BK_PAAS_IP_COMMA", "10.0.0.1")
	t.Setenv("INSPECT_RABBITMQ_STALLED_VHOST_BLACKLIST", "new")
	t.Setenv("INSPECT_RABBITMQ_NO_CONSUMER_VHOST_BLACKLIST", "old")
	c, err := Load("/tmp")
	if err != nil {
		t.Fatalf("Load: %v", err)
	}
	want := []string{"new"}
	if !reflect.DeepEqual(c.Thresholds.RabbitMQStalledVHostBlacklist, want) {
		t.Errorf("blacklist = %v, want %v (new var must win)", c.Thresholds.RabbitMQStalledVHostBlacklist, want)
	}
}
