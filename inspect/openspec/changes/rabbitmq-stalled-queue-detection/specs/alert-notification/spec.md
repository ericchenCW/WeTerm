...
- 形如 `rabbitmq.<vhost>.<queue>.stalled` 的 Field MUST 折叠为
  `rabbitmq.<vhost>.stalled`。
...
旧的 `rabbitmq.<vhost>.<queue>.no_consumer` Field 形态已被废弃，不再参与归一化逻辑（collector/checker 已不再产生）。
...
- **WHEN** 两次巡检均在同一 vhost(如 `bk_bkmonitorv3`)报告 `stalled` 告警,
...
- **WHEN** 同一 vhost 下 `stalled` 队列数量在两次巡检间从 3 个变为 5 个,但 vhost
  本身保持不变
- **THEN** 两次签名 MUST 相同

#### Scenario: RabbitMQ 跨 vhost 新增告警
- **WHEN** 上次仅 vhost A 有 `stalled` 告警,本次新增 vhost B 也出现
  `stalled` 告警
- **THEN** 本次签名 MUST 与上次签名不同

#### Scenario: RabbitMQ backlog 与 stalled 互不合并
- **WHEN** 同一 vhost 上次仅 `backlog` 告警,本次仅 `stalled` 告警
- **THEN** 两次签名 MUST 不同
